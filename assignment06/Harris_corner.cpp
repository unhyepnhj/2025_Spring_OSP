...
int main() {
	...
	int blockSize = 2;    // neighbourhood size
	int ksize = 3;    // Sobel kernel
	...
	normalize(output, output_norm, 0, 1.0, NORM_MINMAX);	// CV_MINMAX -> NORM_MINMAX
	...
	for (int i = radius; i < row+radius; i++) {
		for (int j = radius; j < col+radius; j++) {
			...
			if (input_mirror.at<float>(i, j) > 0) {
				float current = input_mirror.at<float>(i, j);	// ÇöÀç ÇÈ¼¿
				bool isMax = true;	// ÇöÀç ÇÈ¼¿ÀÌ ÃÖ´ñ°ªÀÎÁö 
