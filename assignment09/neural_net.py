...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    s1 = X.dot(W1) + b1  # W1x + b1
    p1 = np.maximum(0, s1) # ReLU
    scores = p1.dot(W2) + b2  # W2x + b2
    ...
    # softmax 
    exp_s = np.exp(scores)  # e^s
    p2 = exp_s / np.sum(exp_s, axis=1, keepdims=True)  # p = e^S/sum(e^s)

    # CE loss
    correct = -np.log(p2[np.arange(N), y])
    data_loss = np.sum(correct) / N
    
    # L2 regularization: sum(W^2) -> data loss + reg loss
    regularization = reg * (np.sum(W1*W1) + np.sum(W2*W2))

    # L = 1/N*sum(L(f(x, W), y)) + reg*R(W)
    ...
    # dL/dscore
    dscores = p2
    dscores[np.arange(N), y] -= 1 # one-hot이므로 z=1, dL/ds_2=p-z
